...
void write_allele_freq(char *outputfilename,int n,int m,struct name namelocus[],int ndigit,int ploidy,
		...
		int **Nallelepl,float ***Ppla,float **Nnielsenpl,float **RA, int K,float **Hopl,float **Hepl,float **hTpl,float **uTpl,
...
void writedistmatrices (char *outputfilename,int n,int m,float givenF,int TypeComp, int *cati,
		...
		float ***corrSlij[10],struct name namei[],struct name namelocus[]);
...
void readsfromfile2(FILE *fp, char *s, char *inputfile, int *line);
